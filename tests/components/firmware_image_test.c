...
TEST(a_valid_application_alone_is_run)
{
    const firmware_image_header_t application = make_header(1024, 0xAAAAu, 5);

    CHECK_EQ_INT(firmware_image_decide_boot(&application, NULL),
                 FIRMWARE_BOOT_RUN_APPLICATION);
}

TEST(nothing_valid_means_recovery)
{
    /* A blank board, or one whose application was erased when the power went.
       Jumping into that would run rubbish. */
    firmware_image_header_t erased;
    memset(&erased, 0xFF, sizeof(erased));

    CHECK_EQ_INT(firmware_image_decide_boot(NULL, NULL), FIRMWARE_BOOT_RECOVERY);
    CHECK_EQ_INT(firmware_image_decide_boot(&erased, &erased), FIRMWARE_BOOT_RECOVERY);
}

TEST(a_staged_image_with_a_different_build_is_installed)
{
    const firmware_image_header_t application = make_header(1024, 0xAAAAu, 5);
    const firmware_image_header_t staged = make_header(2048, 0xBBBBu, 6);

    CHECK_EQ_INT(firmware_image_decide_boot(&application, &staged),
                 FIRMWARE_BOOT_INSTALL_STAGED);
}

TEST(an_older_staged_build_is_still_installed)
{
    /* Deliberate: flashing a known-good older build back in the field must
       work. A "newer only" rule would make that impossible without a cable. */
    const firmware_image_header_t application = make_header(1024, 0xAAAAu, 9);
    const firmware_image_header_t staged = make_header(1024, 0xBBBBu, 2);

    CHECK_EQ_INT(firmware_image_decide_boot(&application, &staged),
                 FIRMWARE_BOOT_INSTALL_STAGED);
}

TEST(the_install_decision_clears_itself)
{
    /*
     * The property that stops the bootloader installing forever. Installing
     * copies the header too, so afterwards the build ids agree — and the same
     * inputs must then say "run".
     */
    const firmware_image_header_t staged = make_header(2048, 0xBBBBu, 6);

    CHECK_EQ_INT(firmware_image_decide_boot(NULL, &staged),
                 FIRMWARE_BOOT_INSTALL_STAGED);

    /* After the copy, the application slot holds exactly what staging held. */
    const firmware_image_header_t application = staged;
    CHECK_EQ_INT(firmware_image_decide_boot(&application, &staged),
                 FIRMWARE_BOOT_RUN_APPLICATION);
}

TEST(staging_over_a_broken_application_is_installed)
{
    /* The recovery path that matters: the application is ruined, but an image
       was staged before the power failed. */
    firmware_image_header_t broken = make_header(1024, 0xAAAAu, 5);
    broken.header_crc32 ^= 1u;

    const firmware_image_header_t staged = make_header(2048, 0xBBBBu, 6);

    CHECK_EQ_INT(firmware_image_decide_boot(&broken, &staged),
                 FIRMWARE_BOOT_INSTALL_STAGED);
}

TEST(a_broken_staged_image_is_ignored)
{
    /* An interrupted upload must never displace a working application. */
    const firmware_image_header_t application = make_header(1024, 0xAAAAu, 5);

    firmware_image_header_t half_written = make_header(2048, 0xBBBBu, 6);
    half_written.header_crc32 ^= 1u;

    CHECK_EQ_INT(firmware_image_decide_boot(&application, &half_written),
                 FIRMWARE_BOOT_RUN_APPLICATION);

    firmware_image_header_t erased;
    memset(&erased, 0xFF, sizeof(erased));
    CHECK_EQ_INT(firmware_image_decide_boot(&application, &erased),
                 FIRMWARE_BOOT_RUN_APPLICATION);
}

TEST(every_combination_of_slot_states_has_a_defined_outcome)
{
    /*
     * Exhaustive over the states a pair of slots can be in, so no combination
     * falls through to something undefined. `same` distinguishes a staged
     * image already installed from a genuinely different one.
     */
    const firmware_image_header_t good_a = make_header(1024, 0xAAAAu, 1);
    const firmware_image_header_t good_b = make_header(1024, 0xBBBBu, 2);

    firmware_image_header_t erased;
    memset(&erased, 0xFF, sizeof(erased));

    struct {
        const firmware_image_header_t *application;
        const firmware_image_header_t *staged;
        firmware_boot_action_t expected;
    } cases[] = {
        { NULL,     NULL,     FIRMWARE_BOOT_RECOVERY },
        { &erased,  NULL,     FIRMWARE_BOOT_RECOVERY },
        { NULL,     &erased,  FIRMWARE_BOOT_RECOVERY },
        { &erased,  &erased,  FIRMWARE_BOOT_RECOVERY },
        { &good_a,  NULL,     FIRMWARE_BOOT_RUN_APPLICATION },
        { &good_a,  &erased,  FIRMWARE_BOOT_RUN_APPLICATION },
        { &good_a,  &good_a,  FIRMWARE_BOOT_RUN_APPLICATION },
        { &good_a,  &good_b,  FIRMWARE_BOOT_INSTALL_STAGED },
        { &erased,  &good_b,  FIRMWARE_BOOT_INSTALL_STAGED },
        { NULL,     &good_b,  FIRMWARE_BOOT_INSTALL_STAGED },
    };

    for (unsigned i = 0; i < count_of_(cases); i++) {
        const firmware_boot_action_t action =
            firmware_image_decide_boot(cases[i].application, cases[i].staged);
        if (action != cases[i].expected) {
            printf("    case %u: expected %s, got %s\n", i,
                   firmware_boot_action_name(cases[i].expected),
                   firmware_boot_action_name(action));
            CHECK(false);
            return;
        }
    }
...
TEST_MAIN(
    ...
    RUN(a_valid_application_alone_is_run);
    RUN(nothing_valid_means_recovery);
    RUN(a_staged_image_with_a_different_build_is_installed);
    RUN(an_older_staged_build_is_still_installed);
    RUN(the_install_decision_clears_itself);
    RUN(staging_over_a_broken_application_is_installed);
    RUN(a_broken_staged_image_is_ignored);
    RUN(every_combination_of_slot_states_has_a_defined_outcome);
)
